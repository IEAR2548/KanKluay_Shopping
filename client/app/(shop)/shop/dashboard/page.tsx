...
//   { day: "Sun", amount: 199 },
// ];
// const MAX_BAR = Math.max(...WEEKLY_SALES.map(d => d.amount));
...
//   const [activeFilter, setActiveFilter] = useState<"Pending" | "To Ship">("To Ship");
...
//   const BAR_HEIGHT = 160; // fixed height for bar area
...
//             📦 Products
//           </div>
//           <div style={styles.sidebarItem} onClick={() => router.push("/shop/orders")}>
//             🧾 Orders
//           </div>
//         </div>
...
//                     <span style={styles.statValue}>$ {TOTAL_SALES.toLocaleString()}</span>
...
//                   <div style={styles.statRow}>
//                     <span style={styles.statValue}>{TOTAL_ORDERS.toLocaleString()}</span>
...
//                         {/* fixed-height wrapper so % height works */}
//                         <div style={{ ...styles.barWrapper, height: BAR_HEIGHT }}>
//                           <div
//                             style={{
//                               ...styles.bar,
//                               height: `${pct}%`,
...
//   // ── FIXED: chart is just a flex row; height is controlled by barWrapper ──
//   chart: {
//     display: "flex",
//     alignItems: "flex-end",   // align cols to bottom baseline
//     gap: 8,
//     borderBottom: "1px solid #eee",
//     paddingBottom: 0,
//   },

//   // each column: amount on top, bar in middle, day label at bottom
//   barCol: {
//     flex: 1,
//     display: "flex",
...
//   // ── FIXED: explicit height so height:% on child bar works ──
//   barWrapper: {
//     width: "100%",
//     display: "flex",
//     alignItems: "flex-end",   // bar grows upward from bottom
//     // height is set inline (BAR_HEIGHT px)
//   },
...
export default function SellerDashboardPage() {
  ...
  const [stats,        setStats]        = useState({
    ...
    sales_diff: 0, orders_diff: 0,
    sales_pct: 0,  orders_pct: 0,
  });
  ...
  const fetchStats = async () => {
    try {
      const res  = await fetch(`${API}/shops/${SHOP_ID}/stats`);
      const json = await res.json();
      if (json.data) {
