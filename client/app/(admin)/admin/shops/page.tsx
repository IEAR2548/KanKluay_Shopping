...
export default function ShopManagementPage() {
  ...
  const [editForm, setEditForm] = useState({ shop_name: '', shop_description: '', status: 'active' });
  ...
  const statCards = [
    { title: 'Total Shop', value: totalShopsCount.toString(), valueColor: 'text-gray-800', stats: getMonthStats(shops) },
    { title: 'Active Shop', value: activeShopsCount.toString(), valueColor: 'text-green-600', stats: getMonthStats(shops.filter(s => s.status === 'active')) },
    { title: 'Inactive', value: inactiveShopsCount.toString(), valueColor: 'text-gray-400', stats: getMonthStats(shops.filter(s => s.status === 'inactive')) },
    { title: 'Suspended', value: suspendedShopsCount.toString(), valueColor: 'text-red-500', stats: getMonthStats(shops.filter(s => s.status === 'suspended')) },
  ];
        ...
        {statCards.map((card, idx) => (
          <div key={idx} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between h-32">
            ...
            <div className="flex items-baseline gap-2 mt-1">
              ...
              <span className={`text-sm font-medium ${card.stats.color} flex items-center`}>
            ...
            <div className="text-xs text-gray-500 mt-1">
              Previous Month <span className={`${card.stats.color} font-medium`}>({card.stats.diffStr})</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-gray-100">
          ...
          <div className="flex items-center bg-[#FEF3C7] rounded-lg p-1">
            ...
            >
              All Shop <span className={activeTab === 'All' ? 'text-green-600' : ''}>({totalShopsCount})</span>
            </button>
            <button 
                ...
                <tr key={shop.shop_id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center justify-center gap-3">
                      <button onClick={() => handleEditClick(shop)} className="text-gray-600 hover:text-blue-600 transition-colors">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(shop.shop_id)} className="text-gray-600 hover:text-red-600 transition-colors">
