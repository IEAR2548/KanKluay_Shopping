...
import { useState, useEffect } from "react";
import { useRouter, useParams } from "next/navigation";
import { useCurrentUser } from "@/lib/hooks/useCurrentUser"; // นำเข้า Hook สำหรับ User จริง
...
function getActiveStep(order: OrderDetail): number {
  if (order.shipping_status === "delivered") return 3;
  if (order.shipping_status === "shipping") return 2;
  // ถ้า order_status ไม่ใช่ pending มักจะแปลว่าจ่ายเงินแล้ว
  if (order.order_status !== "pending") return 1;
  return 0;
}

// Mock tracking timeline (ในระบบจริงอาจจะดึงจาก Table Tracking)
const TRACKING_EVENTS = [
  {
    time: "18-04-2026 18:26",
    title: "การจัดส่งสำเร็จ",
    desc: "พัสดุถูกจัดส่งสำเร็จแล้ว",
    done: true,
    highlight: true,
  },
  {
    time: "",
    title: "ดูหลักฐานการจัดส่งสินค้า",
    desc: "",
    done: true,
    link: true,
  },
  {
    time: "18-04-2026 16:06",
    title: "อยู่ระหว่างการขนส่ง",
    desc: "พัสดุอยู่ระหว่างการนำส่ง",
    done: true,
    highlight: false,
  },
  {
    time: "18-04-2026 14:25",
    title: "พัสดุส่งมอบให้พนักงานขนส่ง",
    desc: "",
    done: false,
    highlight: false,
  },
  {
    time: "18-04-2026 14:03",
...
export default function OrderTrackingPage() {
  ...
  const params = useParams();
  const orderId = params?.orderId as string;
  const { user } = useCurrentUser(); // ดึงข้อมูลผู้ใช้งานปัจจุบัน
  ...
  useEffect(() => {
    ...
    // ดึงข้อมูล Order รายชิ้น
    fetch(`${API}/orders/${orderId}`)
      .then((r) => r.json())
      .then((json) => {
        // รองรับรูปแบบข้อมูลทั้ง { data: { order: ... } } หรือ { data: ... }
        const o = json.data?.order ?? json.data;
        setOrder(o);
      })
      .catch(console.error)
  ...
  if (loading)
    return <div style={styles.loading}>กำลังโหลดข้อมูลการจัดส่ง...</div>;
  if (!order)
    return (
      <div style={styles.loading}>ไม่พบข้อมูลคำสั่งซื้อหมายเลข {orderId}</div>
    );

  const activeStep = getActiveStep(order);
  const isDelivered = order.shipping_status === "delivered";

  // กำหนดวันที่ในแต่ละ Step แบบ Dynamic
  const stepDates = [
    order.order_date,
    order.payment_timestamp || "",
    order.shipping_status !== "pending" ? order.order_date : "",
    isDelivered ? order.payment_timestamp || order.order_date : "",
  ];
      ...
      <div style={styles.layout}>
        <div style={styles.content}>
          ...
          <div style={styles.headerBar}>
            ...
            <span style={styles.orderNumber}>
              ORDER ID : {String(order.order_id).toUpperCase()}
            </span>
            {isDelivered && (
              <span style={styles.deliveredText}>
                | Your order has been successfully delivered.
          ...
          <div style={styles.stepperCard}>
            ...
            <div style={styles.actionArea}>
              <p style={styles.inspectNote}>
                กรุณาตรวจสอบสินค้าทุกชิ้นในคำสั่งซื้อก่อนกดยืนยันการรับสินค้า
                <br />
                หากพบปัญหา คุณสามารถขอคืนเงิน/คืนสินค้าได้ผ่านระบบ
              </p>
          ...
          <div style={styles.bottomRow}>
            ...
            <div style={styles.addressCard}>
              <div style={styles.addressTitle}>Shipping Address</div>
              <div style={styles.addressName}>{order.recipient_name}</div>
              <div style={styles.addressPhone}>{order.address_phone}</div>
              <div style={styles.addressDetail}>{order.address_detail}</div>
            </div>

            {/* Tracking Timeline */}
            <div style={styles.trackingCard}>
              <div style={styles.trackingHeader}>
                <span style={{ fontWeight: 700 }}>KGX Express</span>
                <span style={{ color: "#888", fontSize: 13 }}>
                  TRACKING NO: TH-{order.order_id}99B
                </span>
              </div>
              <div style={styles.timeline}>
                {TRACKING_EVENTS.map((ev, i) => (
                  <div key={i} style={styles.timelineRow}>
                    ...
                    <div style={styles.timelineRight}>
                      ...
                      {ev.desc && (
                        <div style={styles.timelineDesc}>{ev.desc}</div>
                      )}
                    </div>
                  </div>
                ))}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  loading: { padding: 100, textAlign: "center", color: "#666" },
  layout: {
    maxWidth: 1100,
    margin: "24px auto",
    padding: "0 16px",
  ...
  stepDate: { fontSize: 11, color: "#888", textAlign: "center", marginTop: 4 },
  actionArea: {
    display: "flex",
    flexDirection: "column",
    alignItems: "flex-end",
