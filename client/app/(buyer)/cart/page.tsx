...
//   const { user, loading: userLoading } = useCurrentUser(); //ดึง user
...
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { useCurrentUser } from "@/lib/hooks/useCurrentUser"; // ใช้ Hook ที่คุณมี
...
export default function CartPage() {
  const router = useRouter();
  const { user, loading: userLoading } = useCurrentUser(); // ดึง User จากระบบ Auth
  const [groups, setGroups] = useState<GroupedShop[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectAll, setSelectAll] = useState(false);

  // 1. ฟังก์ชันดึงข้อมูล Cart โดยใช้ user.user_id
  const fetchCart = async () => {
    ...
    try {
      ...
      const data: CartData = json.data || { items: [], total_amount: 0 };

      const shopMap: Record<number, GroupedShop> = {};
      if (data.items && data.items.length > 0) {
        for (const item of data.items) {
          const shopId = item.shop_id ?? 0;
          const shopName = item.shop_name ?? "ร้านค้าทั่วไป";
          if (!shopMap[shopId]) {
            shopMap[shopId] = {
              shop_id: shopId,
              shop_name: shopName,
  ...
  // 2. ตรวจสอบสถานะ Login และเริ่มโหลดข้อมูล
  useEffect(() => {
    if (!userLoading) {
      if (user) {
        fetchCart();
      } else {
        // ถ้าโหลด User เสร็จแล้วพบว่าไม่ได้ Login ให้ดีดไปหน้า Login
        router.push("/login");
      }
    }
  }, [user, userLoading]);
  ...
  const changeQty = async (productId: number, newQty: number) => {
    if (newQty < 1 || !user?.user_id) return;
    try {
      await fetch(`${API}/cart/${user.user_id}/items/${productId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ quantity: newQty }),
      });
      fetchCart(); // รีโหลดข้อมูลใหม่หลังแก้ไข
    } catch (err) {
  ...
  const removeItem = async (productId: number) => {
    if (!user?.user_id || !confirm("ต้องการลบสินค้าชิ้นนี้ใช่หรือไม่?")) return;
    try {
      await fetch(`${API}/cart/${user.user_id}/items/${productId}`, {
        method: "DELETE",
      });
  ...
  if (userLoading || (loading && user)) {
    return (
      <div style={styles.loading}>กำลังตรวจสอบสิทธิ์และโหลดข้อมูลตะกร้า...</div>
    );
  }
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* Groups */}
        {groups.length === 0 ? (
          <div style={styles.empty}>ไม่มีสินค้าในตะกร้า</div>
        ) : (
          groups.map((group) => (
            <div key={group.shop_id} style={styles.shopBlock}>
              <div style={styles.shopHeader}>
                ...
                <div key={item.product_id} style={styles.itemRow}>
                  ...
                  <div style={{ ...styles.col, ...styles.qtyControl }}>
                    <button
                      style={styles.qtyBtn}
                      onClick={() =>
                        changeQty(item.product_id, item.quantity - 1)
                      }
                    >
                      -
                    </button>
                  ...
                  <span
                    style={{ ...styles.col, color: "#f5a623", fontWeight: 600 }}
                  >
                    ฿{item.subtotal.toLocaleString()}
                  </span>
                  <div style={styles.col}>
        ...
        <div style={styles.footerBar}>
          ...
          <div style={styles.footerRight}>
            ...
            <button
              style={{
                ...styles.orderBtn,
                backgroundColor: totalItems === 0 ? "#ccc" : "#f5a623",
...
const styles: Record<string, React.CSSProperties> = {
  ...
  totalPrice: { color: "#f5a623", fontWeight: 700, fontSize: 22 },
  orderBtn: {
    border: "none",
    padding: "12px 36px",
    borderRadius: 4,
