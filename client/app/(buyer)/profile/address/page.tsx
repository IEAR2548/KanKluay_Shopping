...
import { useState, useEffect } from "react";
import ProfileSidebar from "@/components/layout/ProfileSidebar";
import { useCurrentUser } from "@/lib/hooks/useCurrentUser"; // นำเข้า hook
...
export default function AddressPage() {
  const { user, loading: userLoading } = useCurrentUser(); // ดึงข้อมูล user จากระบบ login
  const [addresses, setAddresses] = useState<Address[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editAddr, setEditAddr] = useState<Address | null>(null);
  ...
  const load = async () => {
    if (!user) return; // ถ้าไม่มี user ไม่ต้องโหลด
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/users/${user.user_id}/addresses`);
      const json = await res.json();
      setAddresses(json.data || []);
    } catch (e) {
      console.error(e);
  ...
  // โหลดข้อมูลเมื่อ user โหลดเสร็จและมีข้อมูลอยู่จริง
  useEffect(() => {
    if (user) {
      load();
    }
  ...
  // ขณะกำลังโหลด user หรือยังไม่ได้ login
  if (userLoading)
  ...
  const handleSave = async () => {
    try {
      if (editAddr) {
        await fetch(
          `${API_URL}/users/${user.user_id}/addresses/${editAddr.address_id}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(form),
        ...
        await fetch(`${API_URL}/users/${user.user_id}/addresses`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(form),
        });
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("ต้องการลบที่อยู่นี้?")) return;
    await fetch(`${API_URL}/users/${user.user_id}/addresses/${id}`, {
      method: "DELETE",
    });
    load();
  };
  ...
  const handleSetDefault = async (a: Address) => {
    await fetch(`${API_URL}/users/${user.user_id}/addresses/${a.address_id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...a, is_default: true }),
    });
  ...
  return (
    <div className="flex gap-6">
      {/* ใช้ข้อมูลจาก user ที่ login มาแสดงที่ sidebar */}
      <ProfileSidebar username={user.username} imageUrl={user.image_url} />
          ...
          {loading ? (
            ...
            <div className="space-y-5">
              {addresses.map((a) => (
                <div
                  key={a.address_id}
                  className="flex items-start justify-between pb-5 border-b border-gray-100 last:border-0"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
