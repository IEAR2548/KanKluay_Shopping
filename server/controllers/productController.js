...
const getProductsByShop = async (req, res, next) => {
  try {
    const products = await productService.getProductsByShop(req.params.shopId);
    res.json(products);
  } catch (err) {
    next(err);
  }
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          error:
            "shop_id, local_cat_id, product_name, price, quantity are required",
        });
    const product = await productService.createProduct(
      shop_id,
      local_cat_id,
...
const updateProduct = async (req, res, next) => {
  try {
    ...
    if (!product_name || !price)
      return res
        .status(400)
        .json({ error: "product_name and price are required" });
    const product = await productService.updateProduct(
      req.params.id,
      product_name,
      description,
