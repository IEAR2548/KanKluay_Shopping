...
const createGlobalCategory = async (req, res, next) => {
  try {
    ...
    const category = await categoryService.createGlobalCategory(category_name, image_url);
    res.status(201).json(category);
  } catch (err) { next(err); }
};
...
const deleteGlobalCategory = async (req, res, next) => {
  try {
    const category = await categoryService.deleteGlobalCategory(req.params.id);
    if (!category) return res.status(404).json({ error: 'Global Category not found' });
    res.json({ message: 'Global Category deleted successfully', category });
  } catch (err) { next(err); }
};
...
const getLocalCategoriesByShop = async (req, res, next) => {
  try {
    const categories = await categoryService.getLocalCategoriesByShop(req.params.shopId);
    res.json(categories);
  } catch (err) { next(err); }
};
...
const createLocalCategory = async (req, res, next) => {
  try {
    const { shop_id, global_cat_id, category_name, image_url } = req.body;
    if (!shop_id || !global_cat_id || !category_name)
      return res.status(400).json({ error: 'shop_id, global_cat_id, category_name are required' });
    const category = await categoryService.createLocalCategory(shop_id, global_cat_id, category_name, image_url);
    res.status(201).json(category);
  } catch (err) { next(err); }
};
...
const deleteLocalCategory = async (req, res, next) => {
  try {
    const category = await categoryService.deleteLocalCategory(req.params.id);
    if (!category) return res.status(404).json({ error: 'Local Category not found' });
    res.json({ message: 'Local Category deleted successfully', category });
  } catch (err) { next(err); }
};

module.exports = {
  getAllGlobalCategories, createGlobalCategory, deleteGlobalCategory,
  getAllLocalCategories, getLocalCategoriesByShop, createLocalCategory, deleteLocalCategory,
};