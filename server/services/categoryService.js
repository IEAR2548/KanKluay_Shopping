...
const getAllGlobalCategories = async () => {
  const result = await db.query(
    "SELECT * FROM Global_Category ORDER BY global_cat_id",
  );
  return result.rows;
};
...
const getGlobalCategoryById = async (id) => {
  const result = await db.query(
    "SELECT * FROM Global_Category WHERE global_cat_id = $1",
    [id],
  );
  return result.rows[0];
};

const createGlobalCategory = async (category_name, image_url) => {
  const result = await db.query(
    "INSERT INTO Global_Category (category_name, image_url) VALUES ($1,$2) RETURNING *",
    [category_name, image_url || null],
  );
  return result.rows[0];
};
...
const deleteGlobalCategory = async (id) => {
  ...
  try {
    await client.query("BEGIN");
    const localCats = await client.query(
      "SELECT local_cat_id FROM Local_Category WHERE global_cat_id=$1",
      [id],
    );
    for (const lc of localCats.rows) {
      ...
      await client.query("DELETE FROM Product WHERE local_cat_id=$1", [
        lc.local_cat_id,
      ]);
    }
    await client.query("DELETE FROM Local_Category WHERE global_cat_id=$1", [
      id,
    ]);
    const result = await client.query(
      "DELETE FROM Global_Category WHERE global_cat_id=$1 RETURNING *",
...
const createLocalCategory = async (
  ...
  const result = await db.query(
    "INSERT INTO Local_Category (shop_id, global_cat_id, category_name, image_url) VALUES ($1,$2,$3,$4) RETURNING *",
    [shop_id, global_cat_id, category_name, image_url || null],
  );
  return result.rows[0];
};
